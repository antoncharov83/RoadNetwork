...
public abstract class DemoSensor implements TimedItem {
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof DemoSensor)) {
            return false;
        }

        if (threshold != ((DemoSensor) obj).threshold) {
            return false;
        }

        if (Arrays.equals(data, ((DemoSensor) obj).data)) {
            return true;
        }
    ...
    @Override
    public int hashCode() {
        int result = 31*(threshold + currentValue + secondsPassed);
        for (int d : data) {
            result += 17*d;
        }
        return result;
